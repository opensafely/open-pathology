    ...
    has_mtx_rx = (
        medications.where(
            ...
            & medications.date.is_on_or_between(
                INTERVAL.start_date - months(3), INTERVAL.end_date
            )
        ).exists_for_patient()
    ) & (
        medications.where(
            medications.dmd_code.is_in(codelist_mtx)
            & medications.date.is_on_or_between(
                INTERVAL.start_date - months(6), INTERVAL.start_date - months(3)
            )
        ).exists_for_patient()
    )
...
if "diab" in args.test:
    ...
    dmlate_date = (
        prev_events.where(events.snomedct_code.is_in(codelist))
        .sort_by(events.date)
        .last_for_patient()
        .date
    )
    dmreso_date = (
        prev_events.where(events.snomedct_code.is_in(codelist_diab_res))
        .sort_by(events.date)
        .last_for_patient()
        .date
    )
...
if "mean" in args.test:
    ...
    numerator = (
        codelist_events.sort_by(events.date).last_for_patient().numeric_value.as_int()
    )
    denominator = denominator & has_codelist_event

measures.define_defaults(
    numerator=numerator, denominator=denominator, intervals=intervals
)

measures.define_measure(
    name="by_practice",
    group_by={
        "practice": registrations.for_patient_on(INTERVAL.start_date).practice_pseudo_id
    },
)
