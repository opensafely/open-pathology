...
from ehrql import codelist_from_csv, create_dataset, months
...
is_registered = (
    registrations.exists_for_patient_on(study_start_date)
    & registrations.where(
    ...
    has_mtx_rx = (
        medications.where(
            ...
            & medications.date.is_on_or_between(
                study_start_date - months(3), study_end_date
            )
        ).exists_for_patient()
    ) & (
        medications.where(
            medications.dmd_code.is_in(codelist_mtx)
            & medications.date.is_on_or_between(
                study_start_date - months(6), study_start_date - months(3)
            )
        ).exists_for_patient()
    )
