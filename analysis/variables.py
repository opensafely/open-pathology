...
def get_population_variables(start_date=None):
    if start_date is None:
        start_date = INTERVAL.start_date
    age = patients.age_on(date=start_date)
    age_18_to_120 = (age >= 18) & (age < 120)

    registered_practice = practice_registrations.for_patient_on(start_date)
    ...
    died_before_interval_start = has_died & date_of_death.is_before(start_date)
...
def get_measures_variables(start_date=None, end_date=None):
    if start_date is not None and end_date is not None:
        ...
        raise ValueError(
            "start_date and end_date must be either both provided or both empty."
        )
    ...
    for m in key_measures:
        ...
        measures_variables[m + "_binary_flag"] = measures_variables[
            m
        ].exists_for_patient()
